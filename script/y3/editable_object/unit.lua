...
function unit.create_illusion(illusion_unit, call_unit, player, point, direction, clone_hp_mp)
    game_api.create_illusion(illusion_unit.base(), call_unit.base(), player.base(), point.base(), Fix32(direction), clone_hp_mp)
end
...
function unit:add_modifier(modifier_type, source_unit, associated_ability, seconds, duration, stacks)
    self.base():api_add_modifier(modifier_type, source_unit and source_unit.base() or nil, associated_ability and associated_ability.base() or nil, Fix32(seconds), Fix32(duration), stacks)
end
...
function unit:get_mp()
    return self.base():api_get_float_attr("mp_cur")
end
...
function unit:get_life_cycle()
    return self.base():api_get_life_cycle()
end
...
function unit:get_turning_speed()
    return self.base():api_get_turn_speed()
end
...
function unit:get_collision_radius()
    return self.base():api_get_unit_collision_radius()
end
...
function unit:get_unit_selection_range_scale()
    return game_api.get_select_circle_scale(self.base())
end
...
---获取幻象的召唤者
function unit:get_illusion_owner(unit)
    local py_unit = game_api.get_illusion_caller_unit(unit)
    return y3.unit.get_lua_unit_from_py(py_unit)
end
...
function unit:unit_has_item(item)
    return self.base():api_has_item(item)
end
