...
function ui:set_alpha(value)
    game_api.set_ui_comp_opacity(self.player.base(), self.base(), value)
end
...
function ui:get_child_widget_of_specified_name(name)
    local py_ui = game_api.get_comp_by_path(self.player.base(),self.base(),name)
...
function ui:get_parent_widget_of_ui_widget()
    local py_ui = game_api.get_ui_comp_parent(self.player.base(),self.base())
    return y3.ui.get_lua_ui_from_py(self.player ,py_ui)
end
